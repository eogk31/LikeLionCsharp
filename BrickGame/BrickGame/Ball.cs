...
namespace BrickGame
{
    public class Ball
    {
        ...
        public void Progress()
        {
            if (m_tBall.nReady ==0)
            {
                // 0에서 공이 움직일 때 공에 방향 위치에 대한 switch 문
                switch(m_tBall.nDirect)
                {
                    case 0: //위
                        if(Collision(m_tBall.nX, m_tBall.nY -1) ==0)
                            if (m_tBall.nY > 0) m_tBall.nY--;
                        break;
                    case 1: //오른쪽 위
                    ...
                    case 3: // 아래
                        if (Collision(m_tBall.nX, m_tBall.nY + 1) == 0)
                            ...
                            m_tBall.nDirect = g_WallCollision[3, m_tBall.nDirect]; // 충돌 시 방향 변경
                        break;
                    case 4: //왼쪽 아래
                        if (Collision(m_tBall.nX - 1, m_tBall.nY + 1) == 0)
                        {
        ...
        {
            ScreenWall();
            Program.gotoxy(m_tBall.nX, m_tBall.nY); //처리되지 않은 예외
            Console.Write("●");
        }
